  ...
  const getElSecurityAlarm = async () => {
    try{ 
      ...
      if(triggered) {
        ...
        status =  (await el.getPropertyValue(address, eoj, 0xB0)).message.data.value
  ...
  el.on('notify', (res) => {
    ...
    for (const p of prop) {
      ...
      if (p.epc === 0xB1 && p.edt.status)  // status
        updateStatusState(4)
      if (p.epc === 0xB0 && p.edt.value && !status.state != 4){  //
        updateStatusState(estate2h[p.edt.value])
        updateStatusTargetState(estate2h[p.edt.value])
      }
    }
  })
}

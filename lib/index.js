...
class ELPlatform {
      ...
      el.startDiscovery(async (err, res) => {
        ...
        for (const eoj of device.eoj) {
          ...
          if (!el.getClassName(eoj[0], eoj[1]))
            continue
          let uid = address + '|' + JSON.stringify(eoj)
          el.getPropertyValue(address, eoj, 0x83, async (err, res) => {
            if(!err){
              uid = res.message.data ? res.message.data.uid : uid
            }
            const uuid = hap.uuid.generate(uid + eoj[2])
  ...
  async _addAccesory(address, eoj, uuid) {
    ...
    let accessory = registered ? this.accessories.get(uuid)
                        : new Accessory( el.getClassName(eoj[0], eoj[1]) ? el.getClassName(eoj[0], eoj[1])
                            : 'noname' , uuid)
    accessory.getService(hap.Service.AccessoryInformation)
        .setCharacteristic(hap.Characteristic.SerialNumber, address);
